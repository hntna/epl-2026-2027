...
content = re.sub(r'<link rel="preconnect".*?rel="stylesheet">', new_fonts, content, flags=re.DOTALL)
...
:root{
  ...
  --primary:#0F172A; /* Slate 900 (Deep Black/Blue) */
  --primary-dim:#F1F5F9; /* Slate 100 */
  --primary-text:#0F172A; 
  ...
  --shadow-colored:0 10px 15px -3px rgba(15,23,42,0.2);
...
body.dark-mode{
  ...
  --primary-dim:rgba(248,250,252,0.1);
  --primary-text:#F8FAFC; 
  ...
  --shadow-colored:0 8px 16px rgba(248,250,252,0.1);
...
.logo-icon{
  ...
  font-size:18px;background:var(--primary);
...
.nav-item.active .nav-badge{background:var(--bg);color:var(--primary)}
...
.admin-badge.is-admin{background:var(--surface);color:var(--text);border:1px solid var(--border)}
...
.theme-btn:hover{background:var(--surface2);transform:rotate(15deg)}
...
.s-label{font-size:12px;font-weight:600;color:var(--text2);text-transform:uppercase;letter-spacing:0.5px}
.s-value{font-family:var(--font-display);font-size:32px;font-weight:700;line-height:1;color:var(--text);letter-spacing:-1px}
.s-sub{display:none}
...
.rank{font-family:var(--font-display);font-size:16px;font-weight:700;color:var(--text2)}
...
.amount{font-family:var(--font-display);font-weight:700;font-size:14px;letter-spacing:-0.5px}

.empty-state{text-align:center;padding:48px 24px;color:var(--text2);font-size:14px;font-weight:500;background:var(--surface);border:1px dashed var(--border-accent);border-radius:var(--radius)}
.empty-state .ei{font-size:32px;margin:0 auto 12px;opacity:0.5;}

/* TAGS */
.tag{display:inline-flex;align-items:center;gap:4px;font-size:11px;font-weight:600;padding:4px 10px;border-radius:6px;white-space:nowrap}
.tag-stage{background:var(--surface3);color:var(--text)}
...
.match-no{font-family:var(--font-display);font-size:12px;font-weight:700;color:var(--text);background:var(--surface3);padding:4px 8px;border-radius:6px;flex-shrink:0}
...
.mteam-flag{font-size:28px;line-height:1}
.mteam-name{font-size:18px;font-weight:700;color:var(--text);max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:normal;line-height:1.2;word-break:break-word}
...
.vs-txt{font-size:11px;font-weight:700;color:var(--text2);text-transform:uppercase}
.vs-score{font-family:var(--font-display);font-size:28px;font-weight:800;line-height:1;color:var(--text);letter-spacing:-1px}
.vs-handicap{font-size:12px;font-weight:600;color:var(--text2);margin-top:4px;white-space:nowrap}
...
.pred-panel-title{font-size:12px;font-weight:700;color:var(--text2);text-transform:uppercase;letter-spacing:0.5px}
.pred-count-item{font-weight:600}
.pred-locked-bar{background:var(--red-dim);color:var(--red);padding:4px 12px;border-radius:6px;font-weight:600;font-size:12px}
...
.pred-member-row.is-mine{border:1px solid var(--primary);box-shadow:0 0 0 1px var(--primary)}
...
.pred-member-name{display:flex;align-items:center;gap:8px;font-size:13px;font-weight:600;color:var(--text);min-width:0;overflow:hidden}
.me-tag{background:var(--primary);color:var(--bg);font-size:10px;font-weight:700;padding:2px 6px;border-radius:4px;flex-shrink:0}
...
.pbet{
  ...
  font-size:13px;font-weight:600;padding:10px 6px;border-radius:8px;
  cursor:pointer;transition:all .2s;
  white-space:normal; word-break:break-word; line-height:1.2; min-width:0;
  text-align:center; display:flex; align-items:center; justify-content:center;
}
.pbet:hover{background:var(--surface2);color:var(--text)}
.pbet:active{transform:scale(.97)}
.pbet.sel-win{background:var(--green);color:#fff;border-color:var(--green);box-shadow:0 2px 8px rgba(16,185,129,0.3)}
.pbet.sel-draw{background:var(--gold);color:#fff;border-color:var(--gold);box-shadow:0 2px 8px rgba(245,158,11,0.3)}
.pbet.sel-loss{background:var(--red);color:#fff;border-color:var(--red);box-shadow:0 2px 8px rgba(239,68,68,0.3)}
...
.pc-btn{border:none;border-radius:6px;padding:8px 16px;font-size:12px;font-weight:600;cursor:pointer;transition:transform .2s;white-space:nowrap}
.pc-btn:active{transform:scale(.95)}
.pc-confirm{background:var(--primary);color:var(--bg);box-shadow:var(--shadow-sm)}
.pc-unlock{background:var(--surface);color:var(--text);border:1px solid var(--border)}
.pc-badge{font-size:12px;font-weight:600;padding:6px 12px;border-radius:6px;white-space:nowrap}
...
.pc-hint{font-size:11px;color:var(--text2);white-space:nowrap}

.btn-sm{border:none;background:var(--surface);border:1px solid var(--border);color:var(--text);font-size:12px;font-weight:600;padding:6px 12px;border-radius:6px;cursor:pointer;transition:all .2s;white-space:nowrap}
...
.btn-lock,.btn-unlock{border:none;border-radius:6px;padding:6px 12px;font-size:12px;font-weight:600;cursor:pointer;transition:transform .2s;white-space:nowrap}
...
.filter-tab{flex:1;border:none;background:transparent;color:var(--text2);font-size:13px;font-weight:600;padding:8px 16px;border-radius:6px;cursor:pointer;white-space:nowrap;transition:all .2s}
.filter-tab:hover{color:var(--text)}
.filter-tab.active{background:var(--surface);color:var(--text);box-shadow:0 1px 2px rgba(0,0,0,0.05);font-weight:600}

.result-card{background:var(--surface);border:1px solid var(--border);border-radius:var(--radius);margin-bottom:24px;overflow:hidden;box-shadow:var(--shadow-sm)}
.result-card.done{border:1px solid var(--border-accent)}
...
.score-team-name{font-size:15px;font-weight:600;color:var(--text);max-width:100%;overflow:hidden;text-overflow:ellipsis;white-space:normal;line-height:1.2;word-break:break-word}
.score-input-box{display:flex;flex-direction:column;align-items:center}
.score-vs-divider{font-family:var(--font-display);font-size:24px;color:var(--text2);font-weight:700;margin:0 16px}
.score-input-big{width:64px;height:64px;background:var(--surface2);border:1px solid var(--border);border-radius:12px;color:var(--text);font-family:var(--font-display);font-size:32px;font-weight:700;text-align:center;outline:none;transition:all .2s;-moz-appearance:textfield;appearance:textfield;}
.score-input-big::-webkit-outer-spin-button,.score-input-big::-webkit-inner-spin-button{-webkit-appearance:none;margin:0}
.score-input-big:focus{border-color:var(--primary);background:var(--surface);box-shadow:0 0 0 2px var(--primary-dim)}
.score-input-big.s-win{color:var(--green);border-color:var(--green)}
.score-input-big.s-loss{color:var(--red);border-color:var(--red)}

.result-preview-bar{padding:12px 16px;border-radius:8px;background:var(--surface2);font-size:13px;font-weight:600;color:var(--text);text-align:center;margin-bottom:24px;border:1px solid var(--border)}
...
.btn-clear-result{background:var(--surface);color:var(--red);border:1px solid var(--border);font-size:12px;font-weight:600;padding:8px 16px;border-radius:6px;cursor:pointer;transition:background .2s}
...
.member-result-row.mrr-correct{border-left:3px solid var(--green)}
.member-result-row.mrr-wrong{border-left:3px solid var(--red)}
.mrr-name{display:flex;align-items:center;gap:10px;font-size:13px;font-weight:600;color:var(--text);min-width:0;overflow:hidden}
.mrr-pick{font-size:11px;font-weight:600;padding:4px 8px;border-radius:4px;white-space:nowrap;justify-self:start;background:var(--surface2)}
...
.mrr-verdict{font-size:11px;font-weight:600;white-space:nowrap}
.mrr-verdict-correct{color:var(--green)}
.mrr-verdict-wrong{color:var(--red)}
.mrr-penalty{font-family:var(--font-display);font-size:15px;font-weight:700;color:var(--red);text-align:right;white-space:nowrap}
...
.form-section-label{font-family:var(--font-head);font-weight:700;font-size:14px;color:var(--text);margin-bottom:20px;padding-bottom:12px;border-bottom:1px solid var(--border)}
...
.form-group select,.form-group input[type="date"],.form-group input[type="time"]{width:100%;background:var(--surface2);border:1px solid var(--border);color:var(--text);padding:12px 14px;border-radius:8px;font-family:var(--font-ui);font-size:14px;font-weight:500;outline:none;transition:all .2s;min-height:44px}
.form-group select:focus,.form-group input:focus{border-color:var(--primary);background:var(--surface);box-shadow:0 0 0 2px var(--primary-dim)}
...
.bet-btn{width:36px;height:36px;border-radius:6px;flex-shrink:0;border:1px solid var(--border);background:var(--surface);color:var(--text);font-size:20px;font-weight:500;cursor:pointer;display:flex;align-items:center;justify-content:center;transition:all .2s;box-shadow:0 1px 2px rgba(0,0,0,0.05)}
.bet-btn:hover{background:var(--surface2);border-color:var(--border-accent)}
.bet-btn:active{transform:scale(.92)}
.bet-val{font-family:var(--font-display);font-size:20px;font-weight:700;color:var(--text);min-width:100px;text-align:center;letter-spacing:-0.5px}
...
.bb-label{font-size:11px;font-weight:700;letter-spacing:.5px;color:var(--text2);text-transform:uppercase;white-space:nowrap}
.banker-select{background:var(--surface);border:1px solid var(--border-accent);color:var(--text);padding:6px 10px;border-radius:6px;font-size:12px;font-weight:600;outline:none;min-width:150px;cursor:pointer}
.banker-name{font-size:13px;font-weight:700;color:var(--text)}
.banker-name.none{color:var(--muted);font-weight:500;font-style:italic}
.banker-warn{font-size:11px;color:var(--orange);font-weight:600}
.banker-tag{font-size:9px;font-weight:700;background:var(--text2);color:var(--surface);padding:2px 6px;border-radius:4px;margin-left:4px;white-space:nowrap;display:inline-block}
.pred-member-row.is-banker{background:var(--surface2);border-color:var(--border)}
.banker-row-note{margin-left:auto;font-size:11px;font-weight:500;color:var(--text2);font-style:italic;white-space:nowrap}
.member-result-row.mrr-banker{background:var(--surface2);border-color:var(--border)}

/* BUTTONS */
.btn{border:none;border-radius:8px;padding:12px 24px;font-size:13px;font-weight:600;cursor:pointer;transition:transform .2s;min-height:44px;white-space:nowrap}
...
.modal-title{font-family:var(--font-head);font-weight:700;font-size:20px;letter-spacing:-0.5px;color:var(--text);margin-bottom:6px;text-align:center}
.modal-sub{font-size:13px;color:var(--text2);margin-bottom:20px;font-weight:500;text-align:center}
.modal-btns{display:flex;flex-direction:column;gap:10px;margin-top:20px}
.btn-full{width:100%;border:none;border-radius:8px;padding:12px;font-size:14px;font-weight:600;cursor:pointer;transition:transform .2s;min-height:44px}
.btn-full:active{transform:scale(.97)}
.btn-cancel{background:var(--surface2);color:var(--text);border:1px solid var(--border)}
.btn-confirm{background:var(--primary);color:var(--bg);box-shadow:var(--shadow-sm)}
.btn-confirm-red{background:var(--red);color:#fff;box-shadow:0 4px 6px rgba(239,68,68,0.2)}
...
@media(max-width:768px){
  ...
  .nav-item::after{content:'';font-size:10px;font-weight:600;letter-spacing:0;margin-top:2px}
  ...
  .nav-badge{position:absolute;top:4px;right:50%;margin:0;transform:translateX(10px);min-width:16px;height:16px;padding:0 4px;font-size:9px;background:var(--red);color:#fff;border:none;box-shadow:none}
  ...
  #topbar{height:56px;padding:0 16px;gap:8px;position:sticky;top:0;background:rgba(255,255,255,0.85);backdrop-filter:blur(16px);-webkit-backdrop-filter:blur(16px);border-bottom:1px solid var(--border)}
